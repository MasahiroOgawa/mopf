 ...
 */
template <template <typename...> class Container = std::vector, typename Image, typename Functor>
Container<Image> read_training_images(const std::string& fname, std::size_t limit, Functor func) {
 ...
 */
template <template <typename...> class Container = std::vector, typename Image, typename Functor>
Container<Image> read_test_images(const std::string& fname, std::size_t limit, Functor func) {
    Container<Image> images;
    read_mnist_image_file<Container, Image>(images, fname, limit, func);
    return images;
}
 ...
 */
template <template <typename...> class Container = std::vector, typename Label = uint8_t>
Container<Label> read_training_labels(const std::string& fname, std::size_t limit) {
 ...
 */
template <template <typename...> class Container = std::vector, typename Label = uint8_t>
Container<Label> read_test_labels(const std::string& fname, std::size_t limit) {
    Container<Label> labels;
    read_mnist_label_file<Container, Label>(labels, fname, limit);
    return labels;
}
...
MNIST_dataset<Container, Image, Label> read_dataset_3d(const std::string& trainimg_fname, const std::string& testimg_fname,
                                                       const std::string& trainlabel_fname, const std::string& testlabel_fname,
                                                       std::size_t training_limit = 0, std::size_t test_limit = 0) {
    MNIST_dataset<Container, Image, Label> dataset;

    dataset.training_images = read_training_images<Container, Image>(trainimg_fname, training_limit, [] { return Image(1, 28, 28); });
    dataset.training_labels = read_training_labels<Container, Label>(trainlabel_fname, training_limit);

    dataset.test_images = read_test_images<Container, Image>(testimg_fname, test_limit, [] { return Image(1, 28, 28); });
    dataset.test_labels = read_test_labels<Container, Label>(testlabel_fname, test_limit);
 ...
 */
template <template <typename...> class Container, typename Image, typename Label = uint8_t>
MNIST_dataset<Container, Image, Label> read_dataset_direct(const std::string& trainimg_fname, const std::string& testimg_fname,
                                                           const std::string& trainlabel_fname, const std::string& testlabel_fname,
                                                           std::size_t training_limit = 0, std::size_t test_limit = 0) {
    MNIST_dataset<Container, Image, Label> dataset;
...
MNIST_dataset<Container, Sub<Pixel>, Label> read_dataset(const std::string& trainimg_fname, const std::string& testimg_fname,
                                                         const std::string& trainlabel_fname, const std::string& testlabel_fname,
                                                         std::size_t training_limit = 0, std::size_t test_limit = 0) {
