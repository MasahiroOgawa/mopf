...
void MnistDataHandler<Datum, Label>::read(const std::string &datadir) {
  std::cout << "[INFO] try to read mnist data from " << datadir << " @"
            << __func__ << "\n";

  mnist::MNIST_dataset<std::vector, Datum, Label> mnist_dataset =
      mnist::read_dataset<std::vector, std::vector, typename Datum::value_type,
                          Label>(datadir);

  this->dataset_.train_data = std::move(
      mnist_dataset.training_images); // we need this-> to make it dependent
  ...
  std::cout << "[INFO] read "
            << "#train data=" << num_traindata
            << ", #train labels=" << num_trainlbl
            << ", #test data=" << num_testdata
            << ", #test labels=" << num_testlbl << "\n";
...
//---------------------------------------
template <typename Datum, typename Label>
char MnistDataHandler<Datum, Label>::show(const Datum &datum,
                                          const std::string &winname) {
  if (datum.size() != 784)
    throw std::length_error("input data dim: " + std::to_string(datum.size()) +
                            " must be 28*28=784 dim");
...
// in this case, it needs inline specifier. then it has the same address in
// every translation unit. otherwise it cause compile error of multiple
// definition.
template <>
inline char MnistDataHandler<std::vector<unsigned char>, unsigned char>::show(
    const std::vector<unsigned char> &datum, const std::string &winname) {
...
template <>
inline void
MnistDataHandler<std::vector<double>, double>::compute_datamatrices() {
