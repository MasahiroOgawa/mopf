...
namespace mo {
enum class DataType { david = 1, iris, mnist };
...
/// \brief DataHandler
/////////////////////////////
template <typename Datum, typename Label> class DataHandler {
public:
  DataHandler() {}
  virtual void
  read(const std::string &datadir) = 0; ///< pure virtual function; must be
                                        ///< implemented in derived classes.
  virtual char show(const Datum &datum, const std::string &winname) = 0;
  virtual void show_traindata() = 0;
  virtual ~DataHandler() {
  } ///< Because DataHandler has virtual funcs, it needs virtual destructor to
    ///< call derived class's destructor.
...
protected:
  ...
  Matrix train_datamat_;  ///< train_datamat_ = [x1,x2,...,xN]. s.t. xi=[,,,]^T.
                          ///< this is duplicated with Dataset, but needed for
                          ///< Neuralnet because of its datahandling method.
...
/// because in that case we have to define whole inplementation in .h, and which
/// needs mnisthandler.h,
///  so it cause cross include.
//////////////////////////////
std::unique_ptr<DataHandler<std::vector<unsigned char>, unsigned char>>
create_imgdata_handler(const DataType data_type);
...
/// \return
//////////////////////////////
std::unique_ptr<DataHandler<std::vector<double>, double>>
create_vecdata_handler(const DataType data_type);
