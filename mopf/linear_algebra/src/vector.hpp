...
template <int n>
using Vector =
    cv::Vec<double, n>; // opencv only support specific dimension vector.
...
using Point2d = cv::Point2d; // double
...
/// \return
/////////////////////////
template <typename T> double norm(const std::vector<T> &v);
...
double distance(const std::vector<T> &v1, const std::vector<T> &v2,
                const DistanceType dt = DistanceType::l2) {
  if (v1.size() != v2.size()) {
    std::stringstream errmsg;
    errmsg << "v1.size:" << v1.size()
           << " must be equal with v2.size:" << v2.size() << " @distance()";
...
template <typename T>
double cosnorm(const std::vector<T> &v1, const std::vector<T> &v2) {
  return 1 - std::inner_product(v1.begin(), v1.end(), v2.begin(), 0.0) /
                 (norm(v1) * norm(v2));
}
