...
double Hill_climbing::linear_search(F1 f, F2 df, const double &x0) {
  // init variables
  double step = step0_;
  double x = x0;
  if (opt_vis_) {
    ...
    linsearch_vis_
        .show_optimization(); // we have to call this because comp_minmax assume
                              // input points was initially size 1.
  }
  ...
  while (fabs(dfx) > thre_df_) {
    ...
    if (f(x) < f(x_next)) { // in case f(x_next) does not yet reached the
                            // neighborhood of maximal
    ...
    if (opt_vis_) {
      ...
      linsearch_vis_
          .show_optimization(); // note: until minx!=maxx && miny!=maxy, it will
                                // not display image.
    }
