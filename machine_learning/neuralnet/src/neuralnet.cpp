...
Neuralnet::Neuralnet(const Neuralnet& nn)
  : prm_(nn.prm_), vis_{nn.vis_}, id_{nn.id_}, topology_{nn.topology_},
    num_data_{nn.num_data_}, num_layers_{nn.num_layers_},
    in_dim_{nn.in_dim_}, B_{nn.B_}, out_dim_{nn.out_dim_},
    topology_unchange_cnt_{0},
    num_corrected_{nn.num_corrected_}, num_iterate_{nn.num_iterate_},
    J_{nn.J_}, eps_{nn.eps_}, eps_next_{nn.eps_next_},
    Xs_{nn.Xs_}, Ws_hom_{nn.Ws_hom_}, dWs_hom_{nn.dWs_hom_},
    state_{nn.state_}{
...
void Neuralnet::output_log(){
  ...
  if(J_ < -1.0e+4){
    ...
    for(unsigned int l=0;l<num_layers_-1;++l)
      ofs_ << "Ws_hom_[" << l << "]=" << Ws_hom_[l] << endl;
  ...
  if(num_iterate_ == prm_.th_maxit){
    ...
    comp_J();
    ofs_ << "final root mean squared error(id=" << id_ << ", iteration=" << num_iterate_ << ") = " << sqrt(J_)/num_data() << endl;
...
void Neuralnet::show_output(){
  ...
  if(num_iterate_ % vis_.prm().display_step == 0
     || J_ < prm_.th_J) //show anyway at the final step.
  {
    vis_.show_network();
    for(unsigned l=1; l<num_layers_-1; ++l)
      vis_.show_layer(Ys_[l], l);
...
void Neuralnet::update_state(){
  if(J_ < prm_.th_J) state_ = State::solved;
  else if(num_iterate_ >= prm_.th_maxit) state_ = State::overiterate;
  else if(dJ_ < prm_.th_dJ) state_ = State::localminimum;
  else state_ = State::intermediate;
}
...
void Neuralnet::try_create(){
  ...
  if(state_ == State::localminimum && topology_unchange_cnt_ > prm_.th_topology_unchange){
    ...
    state_ = State::intermediate;
    topology_unchange_cnt_ = -1; //the value will be incremented afterward, so at this time -1 to let this value be 0 in case non change.
  }
}
...
void Neuralnet::try_annihilate(){
  ...
  cv::Mat abs_Wsl = abs(Ws_hom_[layer_num](cv::Rect(0,1,Ws_hom_[layer_num].cols, Ws_hom_[layer_num].rows-1))); //exclude bias neuron because we don't want to annihilate the one.
  cv::Mat abssum_neuronweights; //sum of absolute value of each neuron's output weights(exclude bias neuron). = (neuron1's sum of absolute value of output weights; neuron2's ; ...) : (num_neurons-1) x 1 dim
  ...
  if(min_val < prm_.th_annihilate * abssum_allweights){
    annihilate_neuron(layer_num, min_idx[0]+1); //include index of bias neuron which is excluded in calculating min_idx.
    if(vis_.prm().show_opt) vis_.refresh(topology_);
    topology_unchange_cnt_ = -1; //the value will be incremented afterward, so at this time -1 to let this value be 0 in case non change.
  }
}
...
void Neuralnet::annihiate_input_weights(unsigned layer_num, int neuron_num){
  Util util;
  util.delete_col(neuron_num-1, Ws_hom_[layer_num-1]); //note: neuron number(0,1,2,...) corresponds to Ws_hom_'s col number(1,2,3,...). and delete_col use 0 start index.
...
void Neuralnet::read_data(const cv::Mat &X0, const cv::Mat& B){
  ...
  if(static_cast<Problem_type>(prm_.problem_type)==Problem_type::binary_classification && B_.rows!=1) throw runtime_error("dim b must be 1 in binary classification @read_data");
}
...
void Neuralnet::init_weight(){
  ...
  int i=0;
  for(auto w:Ws_hom_)
    ofs_ << "Ws_hom_[" << i++ << "]=" << w << endl;
  i=0;
  for(auto w:dWs_hom_)
    ofs_ << "dWs_hom_[" << i++ << "]=" << w << endl;
#endif
}
...
void Neuralnet::comp_J(){
  ...
  for(unsigned int n=0; n<num_data_; ++n){
    ...
    case binary_classification:
      J_ -= B_.at<double>(0,n) * log(Ys_[num_layers_-1].at<double>(0,n)) + (1-B_.at<double>(0,n)) * log(1-Ys_[num_layers_-1].at<double>(0,n));
      break;
...
void Neuralnet::outlayer_activation(const cv::Mat& X, cv::Mat& Y){
  ...
  case binary_classification:
    ...
    for(int c=0; c<X.cols; ++c)
      Y.at<double>(0,c) = 1/(1+exp(-X.at<double>(0,c)));
    break;
  case multiclass_classification:{
      ...
      for(int c=0; c<X.cols; ++c){
        ...
        for(int r=0; r<X.rows; ++r)
          denominator += exp(X.at<double>(r,c));
        if(denominator==0.0) throw runtime_error("sum exp(xj)==0 @outlayer_activation");
        for(int r=0; r<X.rows; ++r)
          Y.at<double>(r,c) = exp(X.at<double>(r,c)) / denominator;
      }
      break;
    }
  default:
...
void Neuralnet::feed_forward(unsigned data_num){
  ...
  for(unsigned int l=1; l<num_layers_; ++l){
    ...
    for(int i=0;i<y.rows;++i){
      ...
      if(fx<-1 || fx>1){
        ostringstream errmsg("fx<-1 or fx>1!");
        errmsg << ",datanum=" << data_num << ",layernum=" << l << ",fx=" << fx;
...
#ifdef DEBUG2_
    ofs_<<"dfxl("<<layer<<","<<data_num<<")="<<dfxl<<"\n";
    ofs_<<"eps_("<<layer<<","<<data_num<<")="<<eps_<<"\n";
#endif
  }
}
...
void Neuralnet::update_weights(const unsigned layer, const unsigned data_num){
  ...
  if(!util.is_valid(dWs_hom_[layer-1])){
    throw runtime_error("on data_num=" + to_string(data_num) +
                        ", dWs_hom_[" + to_string(layer-1) +
                        "] has invalid values. @Neuralnet::update_weights()");
  }
#ifdef DEBUG2_
...
/**
 * @brief Neuralnet::update_learningrate
 */
void Neuralnet::update_learningrate(){
  prm_.rho *= prm_.rho_descendingrate;
}
